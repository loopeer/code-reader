...
class FileCache {
    ...
    init {
        cacheDir = if (hasSDCard() && hasExternalStoragePermission(CodeReaderApplications().applicationContext)) {
            createFilePath(cachePath)
        } else {
            createFilePath("${CodeReaderApplications().getAppContext()!!.cacheDir}$cacheDirPath")//字符串拼接
        }
    }
