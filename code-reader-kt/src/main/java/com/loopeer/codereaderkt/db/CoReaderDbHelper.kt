...
class CoReaderDbHelper private constructor(context: Context)
    : SQLiteOpenHelper(CodeReaderApplication.appContext, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun insertRepo(repo: Repo): Long {
        ...
        if (repo.lastModify == 0L) repo.lastModify = System.currentTimeMillis()
        val db = writableDatabase
    ...
    private fun haveSameRepo(repo: Repo): Boolean {
        ...
        return if (repo1 != null) true else false
    }
    ...
    fun readSameRepo(repo: Repo): Repo? {
        ...
        val cursor = db.rawQuery(
                ...
                arrayOf<String>(repo.name.toString(), repo.absolutePath.toString()))
    ...
    fun readRepos(): List<Repo> {
        val db = readableDatabase//打开database引起的异常
